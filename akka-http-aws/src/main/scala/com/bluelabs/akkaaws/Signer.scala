...
object Signer {
  ...
  def sessionHeader(creds: AWSCredentials): Option[HttpHeader] = {
    creds match {
      ...
      case sc: SessionCredentials => Some(RawHeader("X-Amz-Security-Token", sc.securityToken))
      case rc: RoleCredentials => Some(RawHeader("X-Amz-Security-Token", rc.sessionToken))
    }
  }
